            ...
            {tab === "description" && (
              <>
                ...
                <div className="grid grid-cols-2 gap-3 rounded-lg bg-slate-50 p-3 border border-slate-200">
                  <div><label className="block text-sm font-medium mb-1">Column</label><select className="w-full px-3 py-2 border rounded-lg" value={taskDraft.column_key} onChange={(e) => setTaskDraft((p) => ({ ...p, column_key: e.target.value }))} disabled={isSaving}>{columns.filter((c) => isEdit || c.key !== "backlog").map((c) => <option key={c.id} value={c.key}>{c.title}</option>)}</select></div>
